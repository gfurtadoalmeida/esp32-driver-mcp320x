...
TEST_CASE("Cannot read with invalid handle", "[read]")
{
    ...
    mcp320x_err_t result = mcp320x_read(NULL, MCP320X_CHANNEL_0, MCP320X_READ_MODE_SINGLE, &value);
...
TEST_CASE("Cannot read with invalid channel", "[read]")
{
    ...
    EXECUTE_WITH_HANDLE(mcp320x_err_t result = mcp320x_read(handle, MCP320X_CHANNEL_7, MCP320X_READ_MODE_SINGLE, &value))
...
{
    EXECUTE_WITH_HANDLE(mcp320x_err_t result = mcp320x_read(handle, MCP320X_CHANNEL_0, MCP320X_READ_MODE_SINGLE, NULL))
...
TEST_CASE("Can read", "[read]")
{
    ...
    EXECUTE_WITH_HANDLE(mcp320x_err_t result = mcp320x_read(handle, MCP320X_CHANNEL_3, MCP320X_READ_MODE_SINGLE, &value))
