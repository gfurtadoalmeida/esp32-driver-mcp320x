...
extern "C"
{
    ...
    TEST_CASE("Cannot read voltage with invalid handle", "[read][voltage]")
    {
        ...
        mcp320x_err_t result = mcp320x_read_voltage(NULL, MCP320X_CHANNEL_0, MCP320X_READ_MODE_SINGLE, 5, &value);
    ...
    TEST_CASE("Cannot read voltage with invalid channel", "[read][voltage]")
    {
        ...
        EXECUTE_WITH_HANDLE(mcp320x_err_t result = mcp320x_read_voltage(handle, MCP320X_CHANNEL_7, MCP320X_READ_MODE_SINGLE, 5, &value))
    ...
    {
        EXECUTE_WITH_HANDLE(mcp320x_err_t result = mcp320x_read_voltage(handle, MCP320X_CHANNEL_0, MCP320X_READ_MODE_SINGLE, 5, NULL));
    ...
    TEST_CASE("Can read voltage", "[read][voltage]")
    {
        ...
        EXECUTE_WITH_HANDLE(mcp320x_err_t result = mcp320x_read_voltage(handle, MCP320X_CHANNEL_3, MCP320X_READ_MODE_SINGLE, 5, &value))
